...
- **Adaptation exercises** — coordinate the vestibulo-ocular reflex (VOR)
  so vision stays focused while the head moves.
- **Gaze stabilization** — move head fast while eyes stay on a target.
- **Target shooting** — head still, eyes moving (or vice versa).
- **Habituation exercises** — repeated controlled exposure to complex
  patterns, busy environments, trigger places (grocery stores, malls) to
  reset nervous-system sensitivity.
- **Balance retraining** — standing on different surfaces, narrowing base
  of support.
- **Balance with eyes closed** — reduce visual dependence, boost inner-ear
  input.
...
- Optokinetic training videos (YouTube links in the VR article): start
  ...
  10-minute sessions, symptoms should clear within 20 minutes.
- Tai chi "cloud hands" movement follows the same habituation principle.
...
- **SSRIs:** fluvoxamine (Luvox®), paroxetine (Paxil®), sertraline
  (Zoloft®).
...
**What does NOT help PPPD (and may slow recovery):**
...
- **Vestibular suppressants** (benzodiazepines, antihistamines like
  betahistine, anticholinergics): reduce acute symptom intensity but slow
  compensation; risks include falls, drowsiness, habit formation,
  withdrawal.
...
- Teaches: noticing/identifying thoughts and beliefs, viewing them from
  different angles, changing behaviour patterns.
...
- **Controlled breathing** — lengthened exhale, abdominal breathing,
  breath focus, alternate nostril breathing. (Caution with breathing
  conditions.)
- **Mindfulness meditation** — research suggests relief from some anxiety
  symptoms comparable to antidepressants for many people. (Caution: ~8%
  experience negative effects.)
- **Progressive muscle relaxation (PMR)** — tensing then releasing muscle
  groups. (Caution with hypertension.)
- **Yoga, tai chi, qigong** — evidence for stress/anxiety reduction; tai
  chi also improves balance and reduces fear of falling.

Tips: quiet dim environment, comfortable clothes, empty stomach, 10–20
minutes daily, be patient — skills take time.
...
- **Vestibular implants** — like cochlear implants for balance; two
  ...
  Johns Hopkins MVI multichannel prosthesis). Targets bilateral
...
- **Wearable biofeedback sensors** — lab/home rehab training and balance
  aids.
- **TIPS (SFU)** — wearable fall detection, hip protectors, compliant
  flooring for seniors.
- **Gene therapy:** hair-cell regrowth restored vestibular function in
  ~70% of mice — proof of concept.
- **Drug repurposing:** bisphosphonates + vitamin D (otosclerosis),
  mitochondrial antioxidants (alpha-lipoic acid, CoQ10) for hair-cell
  protection.
- **VR rehab advances:** mobile sensors, apps, virtual-environment
  training, neuromodulation.
